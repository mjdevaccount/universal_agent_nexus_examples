...
- **Cache Fabric Layer** – Use the shared helpers in `shared/cache_fabric/` instead of ad-hoc caching. The cached content moderation example already routes compiler output, runtime executions, and feedback through this layer while supporting Redis and vector backends.【F:14-cached-content-moderation/run_fabric_demo.py†L19-L129】
- **Observability helper** – Prefer `universal_agent_tools.observability` for tracing instead of per-example wrappers; it integrates cleanly with the Cache Fabric workflow.【F:14-cached-content-moderation/run_fabric_demo.py†L17-L23】【F:14-cached-content-moderation/run_fabric_demo.py†L36-L96】
