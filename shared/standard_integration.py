...
class StandardExample(NexusRuntime):
    ...
    Usage:
        class MyExample(StandardExample):
            ...
            async def execute_example(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
                # Setup if needed
                await self.setup()
                
                # Execute
                result = await self.execute("exec-001", input_data)
                
                # Track and return
                await self.track_execution("exec-001", input_data, result)
                return result
    """
