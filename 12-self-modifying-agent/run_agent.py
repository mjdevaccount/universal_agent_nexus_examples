"""Run the self-modifying agent using LangGraphRuntime with v3.0.1 patterns."""
...
async def main():
    # Setup observability
    obs_enabled = setup_observability("self-modifying-agent")
    ...
    for query in test_queries:
        ...
        if obs_enabled:
            async with trace_runtime_execution(f"self-mod-{query[:10]}", graph_name="main"):
                result = await runtime.execute(
                    execution_id=f"self-mod-{query[:10]}",
        ...
        if messages:
            ...
            for msg in messages:
                if hasattr(msg, 'content'):
                    ...
                    if content_lower in ['search_docs', 'calculate_risk']:
                        routing_decision = content_lower
                    # Find final formatted response
                    elif len(content) > 50 and content_lower not in ['search_docs', 'calculate_risk']:
