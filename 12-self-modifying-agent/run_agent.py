"""Run the self-modifying agent using LangGraphRuntime."""
...
async def main():
    ...
    if obs_enabled:
        async with trace_runtime_execution("self-mod-001", graph_name="main"):
            result = await runtime.execute(
                execution_id="self-mod-001",
                input_data={"context": {"query": "Search documents for risk analysis"}},
            )
    else:
        result = await runtime.execute(
            execution_id="self-mod-001",
